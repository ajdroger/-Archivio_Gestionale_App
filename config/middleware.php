...
return function (App $app) {
    ...
    $app->add(function (Request $request, RequestHandler $handler) use ($guard) {
        ...
        if (($_ENV['APP_ENV'] ?? '') === 'testing' || str_contains($path, '/api/public/') || str_contains($path, '/ai/') || str_contains($path, '/workshift/api/') || str_contains($path, '/taskflow/api/') || str_contains($path, '/expensebar/api/') || str_contains($path, '/soci/calcola-cf')) {
